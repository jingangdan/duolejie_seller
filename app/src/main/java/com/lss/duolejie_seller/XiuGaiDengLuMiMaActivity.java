...
public class XiuGaiDengLuMiMaActivity extends BaseActivity {
	private Context mContext = XiuGaiDengLuMiMaActivity.this;
	...
	Long timexx = null;
				...
				try {
					...
					timexx =System.currentTimeMillis();
			     	String miwen=getSharedPreferenceValue("brand_tel")+"|$|"+"5"+"|$|"+timexx.toString();
	...
	JsonHttpResponseHandler getXiuGaiMiMa = new JsonHttpResponseHandler() {
		...
		public void onSuccess(int statusCode, Header[] headers,
			...
			{
				Toast.makeText(XiuGaiDengLuMiMaActivity.this, "密码修改成功！",Toast.LENGTH_LONG).show();
				XiuGaiDengLuMiMaActivity.this.finish();
				
			} 
			else if (status.equals("111111")) {
				putSharedPreferenceValue("isLoged", "0");
				Toast.makeText(XiuGaiDengLuMiMaActivity.this, "您的账号已在其他地方登录", Toast.LENGTH_LONG).show();
				Intent itziliao = new Intent();
				itziliao.setClass(XiuGaiDengLuMiMaActivity.this, DuoLeMainActivity.class);
				startActivity(itziliao);
			}    else// 失败
			{
				 Tools.Notic(XiuGaiDengLuMiMaActivity.this, desc+"", null);
	...
	JsonHttpResponseHandler getCode = new JsonHttpResponseHandler() {
		...
		public void onSuccess(int statusCode, Header[] headers,
			...
			if (status.equals("0"))// 成功
			{
				Toast.makeText(XiuGaiDengLuMiMaActivity.this, "验证码已发送！",Toast.LENGTH_LONG).show();

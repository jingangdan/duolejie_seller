...
public class WangJiMiMaActivity extends BaseActivity {
	private Context mContext = WangJiMiMaActivity.this;
	private Button btn_queren;
	private TextView txt_yanzhengma, tv_getCode_time;
	private ImageView im_mimaback;
	private EditText  et_phone,et_new_mima1_byYanZhengMa,
	...
	public void onClick(View v) {
		...
		case R.id.txt_yanzhengma:// 获取验证码
			if (!TextUtils.isEmpty(et_phone.getText().toString().trim())) {
				...
				map.put("app_login_id", (et_phone.getText().toString().trim()));
				map.put("type", "5");
	Long timexx = null;
				String jmxx = null;
				try {
					...
					timexx =System.currentTimeMillis();
			     	String miwen=(et_phone.getText().toString().trim())+"|$|"+ "5"+"|$|"+timexx.toString();
				...
				if (TextUtils.isEmpty(text_NewMima1)) {
					...
					try {
				    	...
				    	System.out.println(time);
				     	String miwen=getSharedPreferenceValue("brand_tel")+"|$|"+"2"+"|$|"+text_NewMima1+"|$|"+verify_code+"|$|"+time.toString();
	...
	JsonHttpResponseHandler getXiuGaiMiMa = new JsonHttpResponseHandler() {
		...
		public void onSuccess(int statusCode, Header[] headers,
			...
			{
				Toast.makeText(WangJiMiMaActivity.this, "密码修改成功！",Toast.LENGTH_LONG).show();
				WangJiMiMaActivity.this.finish();
				
			}
			else if (status.equals("111111")) {
				putSharedPreferenceValue("isLoged", "0");
				Toast.makeText(WangJiMiMaActivity.this, "您的账号已在其他地方登录", Toast.LENGTH_LONG).show();
				Intent itziliao = new Intent();
				itziliao.setClass(WangJiMiMaActivity.this, DuoLeMainActivity.class);
				startActivity(itziliao);
			}    else// 失败
			{
				 Tools.Notic(WangJiMiMaActivity.this, desc+"", null);
	...
	JsonHttpResponseHandler getCode = new JsonHttpResponseHandler() {
		...
		public void onSuccess(int statusCode, Header[] headers,
			...
			if (status.equals("0"))// 成功
			{
				Toast.makeText(WangJiMiMaActivity.this, "验证码已发送！",Toast.LENGTH_LONG).show();
